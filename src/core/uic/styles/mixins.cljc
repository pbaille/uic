...
(defn color
  ([x] {:color (css-color x)})
  ([x & mods] (color (cons x mods))))

(defn bg-color
  ([x] {:background-color (css-color x)})
  ([x & mods] (bg-color (cons x mods))))
  ...
  ([x]
   (cond
     ...
     (let [{:keys [dir items content justify
                   grow shrink self basis order gap]} x]
       (flex-return
         (u/remove-nil-vals
           {:flex-direction dir
            :align-self self
     ...
     (apply merge
            ...
            (->> (select-keys opts [:x :y :left :top :bottom :right])
                 (mapv (fn [[dir opts]]
                         (let [opts (cond
                                      (map? opts) opts
                                      (sequential? opts) (zipmap [:width :color :style]
                                                                 opts)
                                      :else {})]
                           (case dir
  ...
  (cond (map? x)
        (str/join " "
                  [(dims/space (:x x 0))
                   (dims/space (:y x 0))
                   (dims/space (:blur x 0))
                   (dims/space (:spread x 0))
                   (css-color (:color x :grey10))
                   (if (:inset x) "inset" "")])
        (sequential? x) (str/join " , " (map ->shadow-value x))
        :else x))

(defn shadow
  ([] (shadow :normal))
  ([x] {:box-shadow (or (get font-shadows x)
                        (->shadow-value x))}))
...
(def sorted-font-sizes [:xs :sm :md :lg :xl :xxl :3xl :4xl :5xl :6xl])
  ...
  ([x]
   (cond
     ...
     (let [{:keys [weight size style family align transform
                   italic non-italic tracking leading decoration break]} x
           ...
           style (or style (and italic :italic) (and non-italic :normal))]
       (u/remove-nil-vals
         (merge
           (font-size size)
           (font-weight weight)
   ...
   (merge (position pos)
          (cond
            ...
            (let [[xt yr & [b l]] placement]
              (if b
                (position pos {:top xt :right yr :bottom b :left l})
                (position pos {:x xt :y yr})))

            (map? placement)
            (merge (placement-spec (select-keys placement [:top :right :bottom :left]))
                   (when-let [xp (:x placement)] (placement-spec {:left xp}))
                   (when-let [yp (:y placement)] (placement-spec {:top yp})))
...
(defn placeholder-color [x]
  {:stylefy.core/manual
   [[:&:placeholder {:color (sc/css-color x)}]]})
