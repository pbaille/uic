...
(def scale-aliases
  {:base 0 :md 0
   :lg 1 :xl 2 :2xl 3 :xxl 3 :3xl 4 :4xl 5
   :sm -1 :xs -2 :2xs -3 :xxs -3 :3xs -4 :4xs -5})
...
(def ratio-aliases
  {:full "100%"
   :double "200%" :triple "300%" :quad "400%"
...
(defn size
  "size is a way to define easily width and height values"
  [x]
  (cond (int? x) (str x "px")
        (float? x) (str (* x 100) "%")
        (contains? ratio-aliases x) (size (ratio-aliases x))
...
#?(:clj (macroexpand-1 '(define-space-mixin margin margin m)))
